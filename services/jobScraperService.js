...
module.exports = new JobScraperService();
  ...
  /**
   * Search jobs from Naukri.com using Puppeteer
   * Reuses the browser instance initialized on server startup
   */
  async searchJobsNaukari({ role, location, experienceLevel, jobType }) {
    ...
    try {
      ...
      if (error.message.includes('Could not find Chrome')) {
        console.log('Chrome not available. Skipping scraping.', error.message);
      }
      return [];
    } finally {
  ...
  /**
   * Main job search function
   * Uses the persistent browser instance
   */
