...
const requestJobSearchLimitExtension = async (req, res) => {
  try {
    ...
    if (user.jobSearchLimitExtensionRequest?.requested && user.jobSearchLimitExtensionRequest?.status === 'pending') {
    ...
    // Update request
    user.jobSearchLimitExtensionRequest = {
      requested: true,
      requestedAt: new Date(),
      message: message || 'Request to extend job search limit',
      status: 'pending'
    };
    ...
    res.status(200).json({
      success: true,
      message: 'Job search limit extension request submitted successfully'
    });
  } catch (error) {
    res.status(500).json({
      success: false,
...
module.exports = {
  getProfile,
  updateProfile,
  requestJobSearchLimitExtension
};
